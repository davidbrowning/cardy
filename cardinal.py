...
try:
    ...
    print("Warning: Some Slates assets not found. Please extract grass.png, platform.png, and goal.png from the tileset.")
    grass_img = None
    platform_img = None
    goal_img = None
...
cardinal_left = pygame.transform.scale(cardinal_left, (40, 40))
cardinal_right = pygame.transform.scale(cardinal_right, (40, 40))
log_img = pygame.transform.scale(log_img, (100, 30))  # Scale log
...
if goal_img:
    goal_img = pygame.transform.scale(goal_img, (40, 40))  # Goal area
...
for i in range(6):  # Create 6 logs
    ...
    log_rect.y = RIVER_TOP + 20 + i * 35  # Space logs vertically in the river
    is_even = -1
    if i % 2 == 0:
        is_even = 1
...
trucks = []
for i in range(3):  # Create 3 trucks
    truck_rect = pygame.Rect(0, 50 + i * 80, 60, 30)  # Simple truck shape
    trucks.append({"rect": truck_rect, "speed": random.choice([-5, 5])})
...
while running:
    ...
    for log in logs:
        ...
        log_hitbox = log["rect"].inflate(10, 10)  # Expand hitbox by 10 pixels on each side
    ...
    if grass_img:
        for x in range(0, WINDOW_WIDTH, 32):
            ...
            for y in range(RIVER_BOTTOM, WINDOW_HEIGHT, 32):  # Bottom section (below river)
                window.blit(grass_img, (x, y))
    else:
    ...
    # Draw yellow lines
    for i in range(3):
        pygame.draw.rect(window, YELLOW, (0, 50 + i * 80, WINDOW_WIDTH, 5))
